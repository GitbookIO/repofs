...
Fs.prototype._move = function move(tree, from, to, msg) {
    ...
    return that.setChange(to, CHANGE.CREATE, msg,
        tree.entries[from].buffer? {
            buffer: tree.entries[from].buffer
        } : {
            sha: tree.entries[from].sha,
            size: tree.entries[from].size,
            mode: tree.entries[from].mode
        }
    )
    .then(function() {
            ...
            that.updateCacheTree(function(tree) {
                ...
                if (entry) {
                    if (!_.isUndefined(entry.buffer) && !_.isNull(entry.buffer)) {
                        entry.size = arrayBuffer.toBuffer(entry.buffer).length;
