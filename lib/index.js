...
Fs.prototype.mergeBranches = function(from, to, opts) {
    ...
    // Remove branch state
    .then(function() {
        return that.getWorkingTree(to).destroy();
    });
};
...
Fs.prototype.resolveConflicts = function(base, head) {
    ...
    .then(function (conflicts) {
        return that._signalConflicts(_.pick(conflicts,
                                           'base', 'head', 'status', 'conflicts'))
        .then(function (resolved) {
            return that.driver.commit({
                message: resolved.message,
                parents: [base, head],
                tree: conflicter.mergeInTree(resolved, conflicts.baseTree)
            });
        });
    });
