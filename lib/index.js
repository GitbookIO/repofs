...
Fs.prototype.stat = refMethod(function stat(p, opts) {
    ...
    .then(function(file) {
        file = that._bindFile(file);

        if (!opts.encoding) file.content = arrayBuffer.enforceArrayBuffer(file.content);
        else file.content = arrayBuffer.enforceString(file.content, opts.encoding);
