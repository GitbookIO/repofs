...
Fs.prototype.fetchTree = decorators.uniqueOp('fetchTree', function fetchTree(opts) {
    ...
    if (tree && _.size(tree.entries) > 0) return Q(tree);
        ...
        _.each(tree.changes, function(change, filePath) {
            ...
            // Create/Update an entry
                ...
                if (change.buffer) {
                    ...
                    tree.entries[filePath].sha = change.sha;
                    tree.entries[filePath].buffer = undefined;
                    tree.entries[filePath].size = change.size;
                }
            }
        });
...
Fs.prototype.write = function write(p, content, opts) {
    ...
    .then(function() {
        that.emitWatch(CHANGE.UPDATE, p);
...
Fs.prototype.create = function create(p, content, opts) {
    ...
    .then(function(exists) {
        ...
        return that.setChange(p, CHANGE.CREATE, opts.message, content);
    })
    .then(function() {
        that.emitWatch(CHANGE.UPDATE, p);
...
Fs.prototype.unlink = function unlink(p, opts) {
    ...
    .then(function() {
        that.emitWatch(CHANGE.REMOVE, p);
...
// Move/Rename
Fs.prototype._move = function move(tree, from, to, msg) {
    var that = this;
    ...
    return that.setChange(to, CHANGE.CREATE, msg, tree.entries[from].buffer? tree.entries[from].buffer : {
        sha: tree.entries[from].sha,
        size: tree.entries[from].size
    })
    .then(function() {
...
Fs.prototype.move = function move(from, to, opts) {
    ...
    return this.operation(
        ...
        function() {
            return that.fetchWorkingTree()
            .then(function(tree) {
                return that._move(tree, from, to, opts.message);
            })
            .then(function() {
                that.emitWatch(CHANGE.CREATE, to);
                that.emitWatch(CHANGE.REMOVE, from);
                ...
                .then(function(tree) {
                    ...
                    return _.reduce(tree.entries, function(prev, entry, entryPath) {
                        ...
                        return prev.then(function() {
                            ...
                            return that._move(tree, entryPath, newEntry, opts.message);
                        });
                    }, Q());
...
Fs.prototype.setChange = function(filePath, type, msg, buffer) {
    ...
    .then(function() {
        return that.operation(msg, function() {
            ...
            that.updateCacheTree(function(tree) {
                if (!tree.changes[filePath]) {
                    ...
                    if (type == CHANGE.UPDATE && tree.changes[filePath].type == CHANGE.REMOVE) {
                     // File has been marked as removed, but is updated; it should be created again first
                        ...
                        that._revertChange(tree, filePath);
                        return false;
                    } else {
                ...
                if (_.isPlainObject(buffer)) {
                    _.extend(tree.changes[filePath], buffer);
                } else {
                    tree.changes[filePath].buffer = buffer? arrayBuffer.toBase64(buffer) : '';
                }
...
Fs.prototype.listChanges = function(ref) {
    var tree = this.getCacheTree(ref);
    return tree.changes || {};
};

// Return true if has uncommited changes
Fs.prototype.hasUncommitedChanges = function(ref) {
    return _.size(this.listChanges(ref)) > 0;
};

// Return change for a file or undefined
Fs.prototype.getChangeForFile = function(filePath) {
...
Fs.prototype.getChangeForFolder = function(dirPath) {
    var changes = this.listChanges();
    var change;
...
Fs.prototype.isFileModified = function(filePath) {
    return Boolean(this.getChangeForFile(filePath));
};

// Revert change on a tree
Fs.prototype._revertChange = function(tree, filePath) {
    if (!tree.changes[filePath]) return;

    var eventType = REVERT_CHANGE[tree.changes[filePath].type];
    ...
    this.setCacheTree(tree);

    // Emit watch event for the revert
    this.emitWatch(eventType, filePath);
};
...
// Revert change for a directory
Fs.prototype.revertFolderChanges = function(dirPath) {
    var that = this;
...
// Revert changes
Fs.prototype.revertChanges = function(type) {
    var that = this;
...
Fs.prototype.checkout = function(branchName, opts) {
    ...
    .then(function(branch) {
        ...
        .then(function() {
            delete that._trees[branch.name];
...
Fs.prototype.pushOp = function(msg) {
    this.updateCacheTree(function(tree) {
...
// Get last operation
Fs.prototype.getLastOperation = function() {
    return this.getCacheTree().lastOperation;
};
...
Fs.prototype.updateCacheTree = function updateCacheTree(fn) {
    var tree = this.getCacheTree();
    var newTree = fn(tree);
    if (newTree === false) return;

    this.setCacheTree(newTree || tree);
};

// Update tree in cache
Fs.prototype.setCacheTree = function updateCacheTree(tree) {
    this.set('tree.'+this.currentBranch, tree);
    delete this._trees[this.currentBranch];
};

// Load tree in cache
Fs.prototype.getCacheTree = function getCacheTree(ref) {
    if (this._trees[ref]) return this._trees[ref];

    var tree = this.get('tree.' + (ref || this.currentBranch), {});
    if (!tree) return null;
