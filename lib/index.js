...
Fs.prototype.doFetchTree = decorators.uniqueOp('fetchTree', function fetchTree(opts) {
    ...
    // Fetch tree
    .then(function() {
        return that.driver.fetchTree(that.currentBranch);
    })
...
Fs.prototype._revertChange = function(tree, filePath) {
    ...
    this.emitWatch(eventType, filePath, true);
};
...
Fs.prototype.commit = function(opts) {
    ...
    var that = this;
    var newCommit; // As returned by the driver after creation
    ...
    this.pendingCommit = this.fetchWorkingTree()
        ...
        .then(function (commit) {
            newCommit = commit;
            return that.driver.updateRef(that.currentBranch, commit.sha);
        })

        // On conflict, attempts an automatic merge
        .fail(function (err) {
            if (err.code === errors.CODE.NOT_FAST_FORWARD) {
                return that.driver.mergeBranches(newCommit.sha, that.currentBranch);
            } else {
                throw err;
            }
        })
        ...
        .fail(function (err) {
            if (err.code === errors.CODE.CONFLICT) {
                return that.resolveConflicts(that.currentBranch, newCommit.sha);
            } else {
                throw err;
            }
        })
...
Fs.prototype.removeBranch = function removeBranch(name) {
    ...
    .then(function() {
        if (!name) throw errors.invalidArgs('Require a branch name');
        if (name == that.currentBranch) throw errors.invalidArgs('Can\'t remove current branch');
        if (that.hasUncommitedChanges(name)) {
