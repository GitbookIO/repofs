...
Fs.prototype.fetchTree = decorators.uniqueOp('fetchTree', function fetchTree(opts) {
    ...
    if (!opts.update) tree = this.getCacheTree(null);
...
Fs.prototype.hasUncommitedChanges = function() {
    return _.size(this.listChanges()) > 0;
};
...
// Load tree in cache
Fs.prototype.getCacheTree = function getCacheTree(def) {
    if (_.isUndefined(def)) def = {};
    var tree = _.cloneDeep(this.get('tree.'+this.currentBranch, def));
