...
function Fs(Driver, opts) {
    ...
    || !this.options.committer.email) throw new Error('Need a valid "committer" as option');
...
Fs.prototype.revertChange = function(ref, filePath) {
    var that = this;
    filePath = normPath(filePath);

    return this.fetchTree(ref)
    .then(function(tree) {
        if (tree[filePath]) return;

        delete tree[filePath].change;
...
Fs.prototype.createBranch = function createBranch(name, from) {
    ...
    return Q()
    .then(function() {
        if (!name) throw new Error('Need a branch name');
...
Fs.prototype.mergeBranches = function mergeBranches(from, to, opts) {
    ...
    return Q()
    .then(function() {
        if (!from) throw new Error('Need a branch to merge');
        if (!to) throw new Error('Need a branch to merge into');
