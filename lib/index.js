    ...
    .then(function(tree) {
        ...
        _.each(tree.changes, function(change, filePath) {
            if (change.type == CHANGE.REMOVE) {
                ...
                workingEntries[filePath].sha = null;
                workingEntries[filePath].buffer = change.buffer;
                workingEntries[filePath].size = change.buffer.length;
            }
        });
...
Fs.prototype.commit = function(opts) {
    ...
    this.pendingCommit = this.fetchTree()
        .then(function(tree) {
...
Fs.prototype.getCacheTree = function getCacheTree() {
    return this.get('tree.'+this.currentBranch, {});
};
