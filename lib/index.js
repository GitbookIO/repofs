...
function Fs(Driver, opts) {
    ...
    || !this.options.committer.email) throw "Need a valid 'committer' as option";
...
Fs.prototype.fetchTree = decorators.uniqueOp('fetchTree', function fetchTree(ref, opts) {
    ...
    if (!opts.update) tree = this.get('tree');
    if (tree) return Q(tree);

    return this.driver.fetchTree(ref)
    .then(function(tree) {
        that.set('tree', tree);
        return tree;
    });
...
// Create a change of type create
Fs.prototype.create = decorators.ref(function create(p, content, opts) {
    ...
    this.setChange(opts.ref, p, 'create', opts.message, content);

    // Signal fs change
    this.emitWatch('add', p);
...
Fs.prototype.setChange = function(ref, path, type, msg, buffer) {
...
Fs.prototype.revertChange = function(ref, path) {
    var changes = this.get('changes', {});

    delete changes[ref][path];
...
Fs.prototype.commit = function(ref, opts) {
    opts = _.defaults({}, opts || {}, {
        message: null
    });

    var changes = this.get('changes', {})[ref] || {};

    // Nothing to commit
    if (_.size(changes) == 0) return Q();
};
