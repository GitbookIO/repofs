...
Fs.prototype._get = function(key, def) {
    var val = this.options.store.get(this.storeKey(key));
    if (_.isUndefined(val) || _.isNull(val)) return def;
    return val;
};

Fs.prototype._set = function(key, val) {
    this.options.store.set(this.storeKey(key), val);
};

Fs.prototype._del = function(key) {
    this.options.store.del(this.storeKey(key));
};
...
Fs.prototype.doFetchTree = decorators.uniqueOp('fetchTree', function fetchTree(opts) {
    ...
    if (tree && _.size(tree.entries) > 0) return Q();
...
Fs.prototype.mergeBranches = function mergeBranches(from, to, opts) {
    ...
    .then(function() {
        ...
        if (that.hasUncommitedChanges(from)) {
            throw errors.invalidArgs('Can\'t merge this branch: there are uncommitted changes');
        }
        if (that.hasUncommitedChanges(to)) {
            throw errors.invalidArgs('Can\'t merge in this branch: there are uncommitted changes');
        }
...
// Update tree in cache
Fs.prototype.setCacheTree = function setCacheTree(tree) {
    this._set('tree.'+this.currentBranch, tree);
    delete this._trees[this.currentBranch];
};

// Remvoe a tree from cache
Fs.prototype.removeCacheTree = function removeCacheTree(ref) {
    this._del('tree.'+ref);
};
...
Fs.prototype.getCacheTree = function getCacheTree(ref) {
    ...
    var tree = this._get('tree.' + (ref || this.currentBranch), {});
    if (!tree) return null;
