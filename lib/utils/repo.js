 ...
 * @param {Branch} branch
 * @param {WorkingState | Null} workingState Pass null to delete
...
function updateWorkingState(repoState, branch, workingState) {
    ...
    if(workingStates === null) {
        ...
        workingStates = workingStates.set(key, workingState);
    }
...
/**
 * Fetch the list of branches in the repository. Existing branch whose
 * SHA have not changed will keep their current WorkingState. The
 * others will need to be fetched again.
 * @param {RepositoryState} repoState
 * @param {Driver} driver
