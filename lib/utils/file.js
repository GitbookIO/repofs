 ...
 * @param {RepositoryState} repoState
 * @param {Path} filepath
 * @return {Boolean} True if the file does not need to be fetched
 */
function isFetched(repoState, filepath) {
    var workingState = repoState.getCurrentState();
    if (WorkingUtils.hasPendingChanges(workingState, filepath)) {
        return true;
    } else {
        var blobSha = WorkingUtils.findSha(workingState, filepath);
        return workingState.getCache().getBlobs().has(blobSha);
    }
}
