...
function fetch(repoState, driver, filepath) {
    ...
    return BlobUtils.fetchBlob(repoState, driver, blobSha);
}
...
function isFetched(repoState, filepath) {
    ...
    return (blobSha === null) || BlobUtils.isBlobFetched(repoState, blobSha);
}
...
function stat(repoState, filepath) {
    ...
    return new File({
        type: FILETYPE.FILE,
        fileSize: treeEntry? treeEntry.getBlobSize() : blob.getByteLength(),
        path: filepath,
        change: change,
        content: blob
    });
