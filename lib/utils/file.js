...
function fetch(repoState, driver, filepath) {
    ...
    .then(function(blob) {
        ...
        return RepoUtils.updateCurrentWorkingState(
            repoState,
            workingState.set('cache', newCache));
    });
}
...
function isFetched(repoState, filepath) {
    ...
    return (blobSha === null) || (workingState.getCache().getBlobs().has(blobSha));
}
 ...
 * @return {Boolean}
 */
function contentChanged(previousState, newState, filepath) {
    if(exists(previousState, filepath) !== exists(newState, filepath)) {
