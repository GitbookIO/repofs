...
// Merges solved conflicts back into a tree. Encodes the entries'
// buffers in base64 and adds their size.
function mergeInTree(resolved, tree) {
    var normConflicts = _.map(resolved.conflicts, function(entry) {
        return {
            path: entry.path,
    ...
    _.merge(tree.entries, normConflicts);
    return tree;
}
