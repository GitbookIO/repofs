...
function mergeInTree(resolved, tree) {
    var normFiles = _.mapValues(resolved.files, function(file) {
        return {
            path: file.path,
            buffer: arrayBuffer.enforceBase64(file.buffer),
            size: _.size(file.buffer)
        };
    });
    var mergedEntries = _.defaults(normFiles, tree.entries);
