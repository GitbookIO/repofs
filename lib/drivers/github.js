...
Driver.prototype.mapFile = function(ref, infos) {
    var rawUrl = infos.download_url || url.resolve(this.options.rawhost, this.options.repository+'/'+ref+'/'+infos.path);
    if (this.options.includeTokenInRaw) rawUrl = rawUrl+'?'+encodeURIComponent(base64.encode(this.options.username + ':' + this.options.token));
...
Driver.prototype.api = function (httpMethod, method, args, options) {
    ...
    var opts = {
        method: httpMethod,
        url: url.resolve(this.options.host, "/repos/"+this.options.repository+"/"+method)+"?t="+Date.now(),
        headers: {
...
Driver.prototype.listCommits = function(opts) {
    ...
    .then(function(commits) {
        return _.map(commits, function(commit) {
            return {
                "sha": commit.sha,
                "author": _.omit(commit.commit.author, "date"),
