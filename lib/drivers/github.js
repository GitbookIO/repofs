...
Driver.prototype.mapFile = function(ref, infos) {
    var rawUrl = infos.download_url || this.rawUrl(ref, infos.path);
    var parts = url.parse(rawUrl, true);
    parts.query.token = parts.query.token || base64.encode(this.options.username + ':' + this.options.token);
    ...
    try {
        infos.content = (_.isString(infos.content) && infos.size < CONTENT_MAX_SIZE)? arrayBuffer.fromBase64(infos.content.replace(/(\r\n|\n|\r)/gm,"")) : undefined;
    } catch (e) {
        infos.content = undefined;
    }
