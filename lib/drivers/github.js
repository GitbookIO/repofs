...
function normalizeError(err) {
    ...
    if(_.includes(err.message, 'fast forward')) {
...
//     parents: [tree.sha],
// }
// Can fail on updating head, with an error marked as
// `needsConflictsResolved` and containing the dangling `commit`
...
Driver.prototype.updateRef = function(ref, sha) {
    return that.api('patch', 'git/refs/heads/'+ref,
                    { sha: sha })
    .fail(normError);
};
