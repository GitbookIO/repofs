...
// Fetch all tree
Driver.prototype.fetchTree = function() {
    return this.api('get', 'git/trees/'+this.fs.currentBranch, {
        recursive: 1
    })
    .then(function(r) {
        return {
...
Driver.prototype.commitChanges = function(branch, base, commit) {
    var that = this, newTree;

    // Get sha of base commit
    return that.api('get', 'git/commits/'+base)
    .then(function(commit) {
        ...
        // Create new blobs
        // todo: http://www.levibotelho.com/development/commit-a-file-with-the-github-api/
        return Q.all(_.map(commit.changes, function(change, filename) {
            return that.api('post', 'git/blobs', {
                content: change.buffer,
                encoding: 'base64'
            })
            .then(function(blob) {
    ...
    .then(function(commit) {
        return that.api('post', 'git/refs/'+branch, {
            sha: commit.sha
        });
    });
};
...
Driver.prototype.checkout = function(branch, opts) {
    ...
    .then(function() {
        ...
        return that.api('post', 'checkout', opts);
    });
};
