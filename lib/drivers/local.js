...
function Driver(options) {
    ...
    if (!this.options.repo) throw "Local driver requires a Gittle 'repo'";

    // GIT repo
    this.repo = this.options.repo;

    // Root folder
    this.root = path.resolve(this.repo.path);
...
Driver.prototype._write = function(p, content, opts) {
    ...
    // Create foldercontainer
    return Q()
    .then(function() {
        var baseFolder = path.dirname(p);
        if (baseFolder == '.') return;

        return Q.nfcall(mkdirp, that.path(baseFolder));
    })

    // Write file
    .then(function() {
        return Q.nfcall(fs.writeFile, that.path(p), content)
    })

    // Commit changes
    .then(function() {
        return that.repo.commitWith(
            that.options.committer.name,
            that.options.committer.email,
            opts.message,
            [p]
        );
    })
...
Driver.prototype.getCommit = function(sha, opts) {
    ...
    return that.repo.getCommit(sha)
...
Driver.prototype.push = function(opts) {
    ...
    return that.repo.push(opts.remote.name || opts.remote.url, opts.branch, opts.auth);
};
