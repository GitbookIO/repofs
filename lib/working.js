...
WorkingTree.prototype.setCurrentBranch = function(name) {
...
// entry.buffer should not be encoded
...
WorkingTree.prototype.move = function move(from, to, msg) {
    ...
    // New entry
    var entry = tree.entries[from];
    if (_.isString(entry.buffer)) entry.buffer = arrayBuffer.fromBase64(entry.buffer);
